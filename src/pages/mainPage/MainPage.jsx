  ...
  return (
    <section className="main w-4/8 flex text-white flex-auto gap-y-5 flex-col">
        ...
        <article className="mb-[20px]">
            ...
            <h2 className="text-xl max-md:text-lg font-medium">Friends ({dataFetch.friend.length||0})</h2>
            ...
            <div className="friend-avatar grid justify-items-center h-40 max-md:h-36 grid-cols-12">
              {dataFetch.isLoading && Array.from(new Array(12)).map((index)=>
              ...
              {!dataFetch.isLoading && dataFetch.friend?.length>0 && dataFetch.friend.map((data, i)=>{
                if(i>11) return
                return (
                    <ButtonDropdown
                    key={data.name}
        ...
        <article>
            ...
            {dataFetch.isLoading && Array.from(new Array(12)).map((index)=>
              ...
              {!dataFetch.isLoading && dataFetch.recommendedForYou?.length>0 && dataFetch.recommendedForYou.map((data)=>{
                return (
                  <ButtonDropdown
                  key={data.name}
